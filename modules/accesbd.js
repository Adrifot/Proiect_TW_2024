...
class AccesBD{
    ...
    initLocal(){
        this.client= new Client({database:"ngshop",
            user:"adrian", 
            password:"admin", 
            host:"localhost", 
            port:5432});
    ...
    select({tabel="",campuri=[],conditiiAnd=[]} = {}, callback, parametriQuery=[]){
        ...
        if(conditiiAnd.length>0)
            conditieWhere=`where ${conditiiAnd.join(" and ")}`; //daca avem conditii in where, le legam cu "and"
...
module.exports = AccesBD; //exportam clasa AccesBD ca pe un modul